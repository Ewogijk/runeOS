...
@click.group(help="""
    Brokkr is the build system for runeOS. It simplifies creating a bootable image and is the recommended way of building
    from sources.
    
    Building works similar to meson, first a build directory must be configured by running './Brokkr configure ...' and
    then the sources can be build using './Brokkr build ...'.
    """)
...
def configure(arch: str, build: str, qemu: bool, freestanding_compiler: str, image_size: int) -> None:
    """Create a build directory for the 'BUILD' builds for the kernel target architecture 'ARCH'.

    FREESTANDING_COMPILER is a path to the installation directory of the freestanding compiler from the runeToolchain.
    It will be used to compile the kernel.

    If QEMU is true, the kernel will be informed that it runs inside the Qemu VM, and it should enable Qemu related
    debugging features.

    IMAGE_SIZE defines the size of the runeOS.image in MB.

    Build configuration involves two steps: First the 'Build/ARCH-DEBUG' directory inside the current directory is
    created, then the 'build.settings' file is created. It is a Json file that contains build parameters for build
    scripts.
    """
    print_banner()
    ...
    build_settings = {
        ...
        Setting.CRT_BEGIN.to_json_key() : str(compiler / 'lib' / 'gcc' / 'x86_64-elf' / '13.2.0' / 'crtbegin.o'),
        Setting.CRT_END.to_json_key() : str(compiler / 'lib' / 'gcc' / 'x86_64-elf' / '13.2.0' / 'crtend.o'),
...
def build_target(arch: str, build: str) -> None:
    """Run the 'Scripts/Build-All.py Build/ARCH-BUILD/build.settings' command to build the kernel, OS, all
    applications in 'Apps/' and create a bootable image.

    Before running the build it is verified that 'Build/ARCH-BUILD/build.settings' exists, if not 'configure' must be
    run first.
    """
    print_banner()

    print(f'Creating build with:')
    print(f'    Arch: {arch}')
    print(f'    Build: {build}')

    build_settings = Path('Build') / f'{arch}-{build}' / 'build.settings'
    if not build_settings.exists():
        sys.exit(f'\'{build_settings}\': Build settings not found. Run \'./Brokkr.py configure ...\' first to create '
                  'a build directory.')
