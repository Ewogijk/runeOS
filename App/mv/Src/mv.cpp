...
int get_node_info(const std::string& node, Ember::NodeInfo& out) {
    ...
    int                     function_ret = -1;
    switch (ret) {
        case Ember::Status::OKAY:           function_ret = 1; break;
        case Ember::Status::NODE_NOT_FOUND: function_ret = 0; break;
        case Ember::Status::BAD_ARG:        std::cerr << "'" << node << "': Bad path." << std::endl; break;
        default:                            std::cerr << "'" << node << "': IO error." << std::endl;
    }
    out = node_info;
    return function_ret;
}
    ...
    if (file_ID < Ember::Status::OKAY) {
        switch (file_ID) {
            ...
            case Ember::Status::BAD_ARG:
                ...
                break;
            default: std::cerr << "'" << node_path << "': IO error." << std::endl; break;
        }
    }
    return file_ID;
}
...
void close_node(const Ember::StatusCode node_ID) {
    if (node_ID <= 0) return; // Invalid node ID
...
bool copy_file_content(const std::string& src, const std::string& dest) {
    ...
    if (dest_node_info.is_directory()) {
        ...
        // We know source is a file, therefore it must contain at least a valid filename ->
        // parts.size() > 0
        dest_node += '/';
        dest_node += str_split(src, '/').back();
    }
    if (!create_node(dest_node, Ember::NodeAttribute::FILE)) return false;

    const Ember::StatusCode src_file_ID = open_node(src, Ember::IOMode::READ);
    if (src_file_ID < Ember::Status::OKAY) return false;
    ...
    if (!delete_node(src)) return false;
...
bool copy_dir_content(const std::string& src, const std::string& dest) {
    ...
    if (!create_node(dest_node, Ember::NodeAttribute::DIRECTORY)) return false;
...
CLINK int main(const int argc, char* argv[]) {
    ...
    if (const int ret = get_node_info(args.src_path, node_info); ret < 1) {
        if (ret == 0) std::cerr << "'" << args.src_path << "': Node not found." << std::endl;
        return -1;
    }

    if (node_info.is_file()) {
        if (!copy_file_content(args.src_path, args.dest_path)) return -1;
    } else {
        if (args.recursive)
            copy_dir_content(args.src_path, args.dest_path);
        else
            std::cerr << "'" << args.src_path << "': Is a directory. Use '-r' to move directories."
                      << std::endl;
    }
    return 0;
}
