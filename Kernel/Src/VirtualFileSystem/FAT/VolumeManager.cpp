...
namespace Rune::VFS {
    size_t VolumeManager::data_cluster_to_lba(
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    U32 VolumeManager::fat_get_eof_marker() const {
    ...
    U32 VolumeManager::fat_read(U16 storage_dev, BIOSParameterBlock* bpb, size_t cluster) const {
        ...
        U8  fat[1024];
        if (_ahci_driver.read(storage_dev, fat, 1024, fat_sector_number) < 1024)
            return 0xFFFFFFFF;
    ...
    bool VolumeManager::fat_write(U16 storage_dev, BIOSParameterBlock* bpb, size_t cluster, U32 fat_value) {
        ...
        if (_ahci_driver.write(storage_dev, fat, 1024, fat_sector_number) < 1024)
            return false;

        memset(fat, 0, 1024);
        U32 fat_backup_sector_number = fat_sector_number + _fat_engine->fat_get_size(bpb);
        if (_ahci_driver.read(storage_dev, fat, 1024, fat_backup_sector_number) < 1024)
            return false;
    ...
    U32 VolumeManager::get_max_cluster_count() const {
        return _fat_engine->get_max_cluster_count();
    }


    bool VolumeManager::data_cluster_read(
            U16 storage_dev,
    ...
    bool VolumeManager::data_cluster_write(
            U16 storage_dev,
            VFS::BIOSParameterBlock* bpb,
            void* buf,
            size_t cluster
