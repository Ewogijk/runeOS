...
namespace Rune::VFS {
    ...
    NodeIOResult FATNode::seek(size_t bytePos) {
        if (_closed)
            return { NodeIOStatus::CLOSED, 0 };
        ...
        if (bytePos >= _file_entry.file.file_size) {
            return { NodeIOStatus::BAD_ARGS, 0 };
        }
        ...
        size_t clusterSize = _mounted_storage->BPB->sectors_per_cluster * _mounted_storage->BPB->bytes_per_sector;
        size_t toSeek      = bytePos;
        U32    cluster     = _file_entry.file.first_cluster_high << 16 | _file_entry.file.first_cluster_low;
        _current_cluster = cluster;
        while (toSeek > 0) {
            if (toSeek < clusterSize) {
                _cluster_offset = toSeek;
                toSeek          = 0;
            } else {
                cluster = _volume_manager.fat_read(_mounted_storage->storage_dev, _mounted_storage->BPB, cluster);
                if (cluster == 0 && cluster >= _volume_manager.get_max_cluster_count() + 1)
                    return { NodeIOStatus::DEV_ERROR, bytePos - toSeek };
        ...
        return { NodeIOStatus::OKAY, bytePos - toSeek };
    }
