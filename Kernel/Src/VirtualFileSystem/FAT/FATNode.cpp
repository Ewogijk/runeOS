...
namespace Rune::VFS {
    ...
    U32 FATNode::processed_bytes() const {
        return _processed_clusters
                   * (_mounted_storage->BPB->bytes_per_sector * _mounted_storage->BPB->sectors_per_cluster)
               + _cluster_offset;
    }

    FATNode::FATNode(Function<void()>             on_close,
                     Path                         path,
    ...
    Path FATNode::get_node_path() const { return _path; }

    Ember::IOMode FATNode::get_io_mode() const { return _node_io_mode; }

    size_t FATNode::get_size() const { return _file_entry.file.file_size; }

    bool FATNode::has_more() const {
        if (_closed) return false;

        if (!has_attribute(Ember::NodeAttribute::FILE)) return false;
    ...
    NodeIOResult FATNode::read(void* buf, size_t buf_size) {
        ...
        if (_file_entry.file.file_size == 0) return {NodeIOStatus::OKAY, 0};

        if (!buf) return {NodeIOStatus::BAD_ARGS, 0};

        if (buf_size == 0) return {NodeIOStatus::OKAY, 0};
        ...
        while (has_more() && buf_pos < buf_size) {
            ...
            size_t b_to_copy = min(min((size_t) _file_entry.file.file_size - processed_bytes(), buf_size - buf_pos),
            ...
            if (_cluster_offset >= cluster_size) {
                U32 next_cluster =
                    _volume_manager.fat_read(_mounted_storage->storage_dev, _mounted_storage->BPB, _current_cluster);
    ...
    NodeIOResult FATNode::write(void* buf, size_t buf_size) {
        ...
        if (_node_io_mode == Ember::IOMode::READ) return {NodeIOStatus::NOT_ALLOWED, 0};
        ...
        while (buf_pos < buf_size) {
            if (_current_cluster == 0
                || _processed_clusters >= div_round_up(_file_entry.file.file_size, (U32) cluster_size)) {
                // End of file reached -> Allocate new cluster
                _current_cluster = _file_entry_manager.allocate_cluster(_mounted_storage->storage_dev,
                                                                        _mounted_storage->BPB,
                                                                        _file_entry,
                                                                        _current_cluster);
                if (_current_cluster == 0) return {NodeIOStatus::DEV_ERROR, 0};
            }
            ...
            if (_cluster_offset >= cluster_size) {
                ...
                U32 next_cluster =
                    _volume_manager.fat_read(_mounted_storage->storage_dev, _mounted_storage->BPB, _current_cluster);
                if (next_cluster < _volume_manager.get_max_cluster_count() + 1) _current_cluster = next_cluster;
            }
        };
        ...
        _file_entry.file.file_size =
            _node_io_mode == Ember::IOMode::WRITE && is_first_write ? buf_pos : _file_entry.file.file_size + buf_pos;
        ...
        if (_file_entry.file.file_size < old_size) {
            ...
            while (cluster != 0 && cluster < _volume_manager.get_max_cluster_count() + 1) {
                U32 next_cluster =
                    _volume_manager.fat_read(_mounted_storage->storage_dev, _mounted_storage->BPB, cluster);
    ...
    NodeIOResult FATNode::seek(const Ember::SeekMode seek_mode, const int offset) {
        ...
        const size_t cluster_size =
            _mounted_storage->BPB->sectors_per_cluster * _mounted_storage->BPB->bytes_per_sector;
        ...
        switch (seek_mode) {
            case Ember::SeekMode::BEGIN:
                ...
                break;
            case Ember::SeekMode::CURSOR: bad_offset = file_cursor + offset >= _file_entry.file.file_size; break;
        ...
        switch (seek_mode) {
            case Ember::SeekMode::BEGIN:  to_seek = offset; break;
            case Ember::SeekMode::CURSOR: to_seek = file_cursor + offset; break;
            case Ember::SeekMode::END:    to_seek = _file_entry.file.file_size + offset; break;
            default:                      break;
        }
    ...
    bool FATNode::has_attribute(Ember::NodeAttribute f_attr) const {
        ...
        switch (f_attr) {
            case Ember::NodeAttribute::READONLY:  fat_attr = FATFileAttribute::READONLY; break;
            case Ember::NodeAttribute::HIDDEN:    fat_attr = FATFileAttribute::HIDDEN; break;
            case Ember::NodeAttribute::SYSTEM:    fat_attr = FATFileAttribute::SYSTEM; break;
            case Ember::NodeAttribute::DIRECTORY: fat_attr = FATFileAttribute::DIRECTORY; break;
            case Ember::NodeAttribute::FILE:      fat_attr = FATFileAttribute::ARCHIVE; break;
            default:                              fat_attr = FATFileAttribute::NONE;
        }
    ...
    bool FATNode::set_attribute(Ember::NodeAttribute n_attr, bool val) {
        if (_closed) return false;
        if (n_attr == Ember::NodeAttribute::FILE || n_attr == Ember::NodeAttribute::DIRECTORY) return false;
        if (val)
