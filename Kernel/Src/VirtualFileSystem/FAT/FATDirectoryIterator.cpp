...
namespace Rune::VFS {
        ...
        if (_current_entry->attributes == FATFileAttribute::LONG_FILE_NAME) {
            ...
            while (c_order > 0) {
                ...
                if (c_order == lfne_count) {
                    ...
                    while (tmp[pEnd - 1] == (char) 0xFF || tmp[pEnd - 1] == '\0') // string will implicitly be null terminated
                        pEnd--;
                } else if (c_order == 1) {
    ...
    NavigationResult FATDirectoryIterator::navigate_to(
            U16 storage_dev,
            BIOSParameterBlock* bpb,
            const VolumeManager& volume_manager,
            U32 start_cluster,
    ...
    LocationAwareFileEntry& FATDirectoryIterator::operator*() {
    ...
    FATDirectoryIterator& FATDirectoryIterator::operator++() {
        ...
        switch (_it_mode) {
            ...
            case DirectoryIterationMode::ATOMIC:
                // We return each file entry individually (no LFN entry parsing) -> LocationAwareFileEntry already set by "advance"
            case DirectoryIterationMode::NONE:
                break;
        }
    ...
    DirectoryIteratorState FATDirectoryIterator::get_state() const {
        return _state;
    }


    U32 FATDirectoryIterator::get_current_cluster() const {
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
    ...
    void FATDirectoryStream::update_state() {
        switch (_fat_it.get_state()) {
            case DirectoryIteratorState::ITERATING:
            ...
            case DirectoryIteratorState::DEV_ERROR:
                _state = DirectoryStreamState::IO_ERROR;
                break;
            default:
                _state = DirectoryStreamState::NONE;
                break;
        }
    }


    FATDirectoryStream::FATDirectoryStream(
