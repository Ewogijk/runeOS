...
namespace Heimdall {
    ...
    struct ReporterRegistry::ReporterListDetail {
        Rune::LinkedList<Rune::SharedPointer<Reporter>> list;
    };
    ...
    auto ReporterRegistry::operator=(ReporterRegistry&& other) noexcept -> ReporterRegistry& {
        if (this == &other) return *this;
        ReporterRegistry tmp(Rune::move(other));
        swap(*this, tmp);
        return *this;
    }
    ...
    void ReporterRegistry::insert(Reporter* reporter) {
        Rune::SharedPointer<Reporter> up(reporter);
        _list_detail->list.add_back(up);
    }
