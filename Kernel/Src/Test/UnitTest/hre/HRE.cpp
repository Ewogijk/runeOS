...
namespace Heimdall {
    ...
    void hre_save_test_report(const HString& file, const HString& test_report) {
        ...
        if (st != Rune::VFS::IOStatus::CREATED) {
            hre_log_emergency(file + ": Could not create test report file. Reason: " + st.to_string() + "\n");
            return;
        }
        ...
        if (io_res.status != Rune::VFS::NodeIOStatus::OKAY) {
            hre_log_emergency(file  + ": Could not write test report. Reason: " + io_res.status.to_string()+ "\n");
            return;
        }
        node->close();
    }
