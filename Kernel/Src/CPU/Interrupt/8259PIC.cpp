...
namespace Rune::CPU {
    ...
    String _8259PIC::get_name() {
        return "8259 PIC";
    }


    U8 _8259PIC::get_irq_line_offset() {
        return ICW2::PIC1_IRQ_OFFSET;
    }
    ...
    bool _8259PIC::is_irq_masked(U8 irq_line) {
        return check_bit(get_imr_0(), irq_line);
    }
    ...
    void _8259PIC::send_end_of_interrupt(U8 irq_line) {
        if (irq_line >= 8)
            out_b(Port::COMMAND2, Command::EOI);
