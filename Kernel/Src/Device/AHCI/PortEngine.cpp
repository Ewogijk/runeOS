...
namespace Rune::Device {
    ...
    HardDrive PortEngine::get_hard_drive_info() const {
        return _disk_info;
    }


    bool PortEngine::is_active() const {
        return _port && _port->CMD.ST && _port->CMD.FRE;
    }
    ...
    bool PortEngine::start(
        ...
        _internal_buf_cache = _heap->create_new_cache(
                Request::INTERNAL_BUF_SIZE,
                2,
                true
        );
        ...
        if (s48_bit) {
            _disk_info.sector_count = ((U64) buf[103] << 48)
                                      | ((U64) buf[102] << 32)
                                      | ((U64) buf[101] << 16)
                                      | (U64) buf[100];
        } else {
        ...
        GPTScanResult                       scan_res     = gpt_scan_device(
                _logger,
                sectorReader,
                _disk_info.sector_size
        );
        ...
        if (scan_res.status == GPTScanStatus::DETECTED) {
            for (auto& partition: scan_res.partition_table) {
                bool is_kernel_partition = memcmp(
                        partition.unique_partition_guid.buf,
                        (void*) KERNEL_PARTITION_GUID,
                        GUID::SIZE
                ) == 0;
                _disk_info.partition_table
                          .add_back(
                                  {
                                          partition.get_name(),
    ...
    size_t PortEngine::send_ata_command(void* buf, size_t bufSize, RegisterHost2DeviceFIS h2dFis) {
...
#ifdef IS_64_BIT
        if (_s64_a)
            ct.PRDT[0].DBAU = (U32) (p_internal_buf >> 32);
#endif
        ...
        if (_system_memory->CL[slot].W)
            memcpy(request.internal_buf, request.buf, request.buf_size);
