...
namespace Rune::CPU {
    ...
    union SegDAccessByte {
        ...
        struct {
            ...
            U8 read_write : 1; // Code - 0: No read, 1: Read allowed | Data - 0: Read only, 1: Write
                               // allowed
            U8 direction_conforming : 1; // Code - 0: Exec =DPL, 1: Exec >=DPL | Data - 0: Grow up,
                                         // 1: Grow down
    ...
    struct TaskStateSegment64 {
        ...
        // Stack pointers that will be loaded when the CPU switches from Ring3 to a lower Ring e.g.
        // Ring3 -> Ring0, then load RSP0. That is the only one we are using.
        U64 rsp_0;
        U64 rsp_1;
        U64 rsp_2;
...
#define GDT_OFFSETS(X)                                                                             \
    ...
    /**
     * @brief Setup a null descriptor, kernel mode code and data segment, user mode code and data
     * segment and the given task state segment in the specified GDT.
     *
