...
#define VMMStartFailures(X)                                                                                            \
    ...
    /**
     * The Virtual Memory Allocator is responsible for allocating/freeing pages in a virtual address space and managing
     * whole virtual address spaces (VAS) which includes creating new VAS's and swapping a current VAS for another
     * existing VAS.
     */
    class VirtualMemoryManager {
        ...
        // Map a coherent physical address range to a virtual address range and claim the region in the virtual memory
        // map
      ...
      public:
        ...
        /**
         * Load the initial virtual address space (VAS) using the given physical memory map, virtual memory map and
         * virtual kernel space layout. The VAS will contain mappings for the higher half direct map (HHDM) of the
         * physical memory, kernel code, physical memory manager and kernel heap.
         *
         * <p>
         *  The PMM reserved and kernel code area sizes are determined by the sizes of the pmm memory index and kernel
         *  size. Whereas the size of the HHDM is determined by the physical memory address range, this means the size
         *  of the HHDM will be the largest possible physical address. Lastly the kernel heap will have the requested
         *  `kernelHeapSize`.
         * </p>
         *
         * <p>
         *  The starting addresses of said regions is determined by the given `kSpaceLayout` and combined with the
         * sizes, the resulting areas will be claimed in the given `VMap`. The `PMap` is used to get the locations of
         * all regions in the physical memory.
         * </p>
         *
        ...
        [[nodiscard]]
        VMMStartFailure
        start(MemoryMap* p_map, MemoryMap* v_map, KernelSpaceLayout k_space_layout, MemorySize heap_size);
        ...
        /**
         * allocate a new virtual address space containing the kernel space index page tables and an empty user space.
         * If the allocation succeeds the physical address of the base page table will be assigned to `basePTAddr`.
         *
         * @param base_pt_addr On success the physical address of the created base page table will be assigned to this
         *                      address else the value of the variable is undefined.
         *
        ...
        /**
         * free the user space of the virtual address space determined by the base page table loaded at `basePTAddr`.
         *
        ...
        /**
         * Try to allocate a page in the base page table loaded by the CPU for the page aligned `vAddr` using the
         * `pFlags` to a page frame which will be requested from the physical memory manager.
         *
        ...
        /**
         * Try to allocate `pages` pages in the base page table loaded by the CPU for the page aligned `vAddr`
         * using the `pFlags` to a page frame which will be requested from the physical memory manager.
         *
         * @param vAddr Virtual address of a page to map to a page frame.
         * @param flags Page flags.
        ...
        /**
         * Try to free the page aligned `vAddr` in the base page table loaded by the CPU by removing the mappings in
         * the page tables.
         *
        ...
        /**
         * Try to free `pages` pages aligned `vAddr` in the base page table loaded by the CPU by removing the mappings
         * in the page tables.
         *
         * @param vAddr Virtual address of a page to free.
