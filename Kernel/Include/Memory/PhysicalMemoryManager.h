...
#define PMM_START_FAILURES(X)                                                                      \
    ...
    /**
     * The physical memory manager allocates and frees page frames. It also ensures that reserved
    ...
    class PhysicalMemoryManager {
      ...
      public:
         ...
         * <ol>
         *  <li>Detect the pmm managed memory range defined by the lowest and highest address in the
         * physical memory map.</li> <li>Search the first usable memory region that has enough space
         * to fit the memory index (MI).</li> <li>Initialization of the MI which is implementation
         * dependent.</li> <ol>
         *
         * <p>
         * The `memoryIndexOffset` is the memory offset towards the virtual memory address that will
         * be used by the pmm to access the MI.
         * </p>
         *
         * @param pMap              Physical memory map.
         * @param page_size          Page frame size.
         * @param memory_index_offset Address offset until the address that will be used to access
         * the memory index.
         *
         ...
         * size and if the second initialization phase ended successfully it will also contain a
         * valid start address.
         *
         ...
         *
         * @param memory_index Start address of the memory region where memory index can be
         * accessed.
         */
         ...
         * given pAddr.
         *
         * @param p_addr The start address of the page frame will be saved to the variable on
         ...
         * consecutive in the memory. The start address of the memory region is saved to the pAddr
         * variable.
         *
         * @param pAddr  The start address of the page frame will be saved to the variable on
         * successful allocation.
         ...
         * address which are guaranteed to be consecutive in the memory.
         *
         * @param pAddr The start address of the page frame will be saved to the variable on
         * successful allocation.
         * @param frames Number of requested page frames.
         ...
         * `buf` until `bufSize` regions have been inserted. Consecutive regions free or used
         * regions will be merged to a single region before being inserted into the `buf`.
         *
         * <p>
         * The memory regions inserted into the buffer may not fully represent the requested memory
         * region if the `start` is not page frame aligned it will be rounded down to the next page
         * frame aligned memory address, the `end` address will be rounded up to the next page frame
         * aligned memory address.
         * </p>
         *
         * <p>
         * If `start` or `end` lie outside of the pmm managed memory region than nothing will be
