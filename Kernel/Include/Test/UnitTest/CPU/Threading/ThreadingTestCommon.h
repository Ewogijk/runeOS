...
/// @brief RAII style
struct TestThread {
    Ember::Handle m_thread_handle = 0;
    // SharedPointer< Ember::ThreadLaunchPacket> m_start_info;
    bool m_sync_thread_stop;

    TestThread(const String& name, Ember::ThreadMain thread_main, bool sync_thread_stop)
        ...
        m_thread_handle = cpu_module->schedule_new_thread(
            name,
            ThreadLaunchPacketBuilder().main(thread_main).build(),
    ...
    ~TestThread() {
        auto* cpu_module = System::instance().get_module<CPU::CPUModule>(ModuleSelector::CPU);
        if (m_thread_handle != Resource<Ember::Handle>::HANDLE_NONE) {
