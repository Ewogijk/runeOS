...
namespace Rune::CPU {
    /**
     * A round robin scheduler that utilizes a multilevel queue with each queue having a scheduling policy that
     * determines the priority of threads in that queue.
     *
     * <p>
     *  A thread terminator is used to free memory of terminated threads as well as a idle thread that halts the CPU
     *  until another thread is ready to run. The thread terminator has implicitly the highest priority while the idle
     *  thread the lowest, both are not scheduled in the multilevel queue.
     * </p>
     */
    class Scheduler {
        ...
        /**
         * @brief If (_allowPreemption == true) threads can be preempted. Note: We cannot know if a schedule() call
         *          happens as part preemption, thus we cannot enforce this rule and must hope preemption is implemented
         *          properly.
         */
         ...
         * thread from the ready queue will be chosen. Only if the ready queue is empty the idle thread
         * will be returned.
         *
    ...
    public:
         ...
         *
         * The thread terminator is the second thread to be created during kernel boot thus it will always have ID 1 and
         * furthermore it never terminates.
         *
         ...
         *          finished execution.
         *
         * The thread terminator is the third thread to be created during kernel boot thus it will always have ID 2 and
         * furthermore it never terminates.
         *
         ...
         * <ol>
         *  <li>The Bootstrap thread is created and set as the running thread, as it was running implicitly since the
         *          computer was turned on.</li>
         *  <li> The idle thread is created.</li>
         ...
         * for disabling interrupts and postponing context switches. Both locks can be acquired multiple times.
         */
        void lock();
         ...
         *
         * This function only sets the KernelStackBottom, KernelStackTop and State field of the thread, the other fields
         * must be set before passing the thread to this function.
         *
         * @param thread New thread to be scheduled.
         * @return True: The thread is in the ready queue waiting to be scheduled, False: The thread has policy "None"
         ...
         *
         * <p>
         *  This function is intended to schedule an already initialized thread that was blocked / waiting in another
         *  wait queue and should now be scheduled for execution.
         * </p>
         * <p>
         *  It is the callers responsibility to remove the thread from it's wait queue before calling this function.
         * </p>
         *
         * @param thread
         * @param preemptRunningThread
         * @return True: The thread is in the ready queue, False: The could not be enqueued in the ready queue, it was
         ...
         *          allowed meaning not postponed.
         *
         * <p>
         *  Important note: It is the callers responsibility to lock/unlock the scheduler before/after calling this
         *  function.
         * </p>
         * <p>
         *  If there are any terminated threads, then the thread terminator is scheduled. Otherwise the next thread
         *  in the highest available scheduling policy of the ready queue is scheduled. Should no thread be ready for
         *  scheduling the idle thread will be scheduled.
         * </p>
         * <p>
         *  When the currently running thread is still in the "Running" state and no other thread is ready then it will
         *  be rescheduled otherwise not.
         * </p>
         * <p>
         ...
         *
         * <p>
         *   It is the callers responsibility to make sure that the thread gets removed from it's current wait queue,
         *   before it is put in the terminated thread queue.
         * </p>
         * <p>
         *  Important note: It is the callers responsibility to lock/unlock the scheduler before/after calling this
         *  function.
