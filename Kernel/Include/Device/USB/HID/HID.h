...
#define HID_PROTOCOL_MODES(X)                                                                      \
    ...
    /// @brief Inhibit reporting on the INTERRUPT IN endpoint forever.
    constexpr U8 HID_SET_IDLE_DURATION_INDEFINITE = 0;

    /// @brief Apply the idle rate for all reports generated by the HID device.
    constexpr U8 HID_IDLE_ALL_REPORTS = 0;

    /// @brief Create a hid SET_PROTOCOL control transfer request.
    /// @param hid_device
    ...
    /// @brief Create a hid SET_IDLE control transfer request.
    /// @param hid_device
    /// @param interface_number
    /// @param report_ID
    /// @param duration_ms
