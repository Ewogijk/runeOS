...
namespace Rune {
    ...
    class CriticalSection {
      ...
      public:
        explicit CriticalSection(const SharedPointer<LockType>& lock) pre(lock)
            : _lock_ptr(lock.get()) {
            _lock_ptr->lock();
        }
        explicit CriticalSection(LockType& lock) : _lock_ptr(&lock) { _lock_ptr->lock(); }
        ~CriticalSection() { _lock_ptr->unlock(); }

        CriticalSection(const CriticalSection<LockType>& other) = delete;
        CriticalSection(CriticalSection<LockType>&& other)      = delete;
        auto operator=(const CriticalSection<LockType>& other)
            -> CriticalSection<LockType>&                                               = delete;
        auto operator=(CriticalSection<LockType>&& other) -> CriticalSection<LockType>& = delete;
    };
} // namespace Rune::CPU
