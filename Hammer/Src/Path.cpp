...
namespace Rune {
    ...
    LinkedList<String> Path::split() const {
        return _path.split(UNIX_PATH_SEPARATOR);
    }


    Path Path::append(const String& part) const {
        if (_path.size() == 0 && part.size() == 0)
            return { };
        if (_path.size() == 0)
            return Path(part);
        if (part.size() == 0)
    ...
    Path Path::operator/(String&& part) const {
        return append(part);
    }


    Path Path::operator/(const Path& part) const {
        return append(part.to_string());
    }
